...
const Plans = () => {
    ...
    function loadPlans(){
        setLoading(true)
        axios.get(API_ENDPOINT+'admin/get-plans', getHeader()).then(res=>{
            setLoading(false)
            const d = res.data
            if(d.success){
                d.message.sort((a, b) => {
    ...
    return (
        <div>
            <MyModal showModal={showAdd} title="Add Plan" setShowModal={setShowAdd} modalC={<AddPlan show={showAdd} onActionDone={onActionDone} mode={currentMode} plan={currentPlan} />}/>
            <CustomCircularP show={loading}/>
        <div style={{display: 'flex'}}>
            <div style={{flex: 1}}><h2>Plans</h2></div>
            <div style={{alignSelf: 'center'}}><Button onClick={addPlan} variant='contained' startIcon={<AccountBalanceWalletIcon />}>Add Plan</Button> </div>
        </div>
