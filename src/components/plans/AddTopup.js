...
const AddTopup = ({plan, mode, onActionDone, show}) => {
    ...
    const onAdd = ()=>{
       ...
       if(name === ''){
        ...
        toast.error("Please enter count")
            ...
            setLoading(true)
            axios.post(API_ENDPOINT+`admin/${mode==="add"?"add-topup":"update-topup"}`, dataObj, getHeader()).then(res=>{
                const d = res.data
                setLoading(false)
                if(d.success){
    ...
    return (
        ...
        <TextField value={name} onChange={e=>setName(e.target.value)} sx={{marginTop: '16px'}} inputMode='text' fullWidth variant='outlined' size='small' label='Name'/>
        <TextField value={price} onChange={e=>setPrice(e.target.value)} sx={{marginTop: '16px'}} type='number' fullWidth variant='outlined' size='small' label='Price'/>
        ...
        <ConfigItem text='Is Topup Active?' checked={isActive} setChecked={setIsActive}/>
